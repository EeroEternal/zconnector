...
fn runOpenAi(init: std.process.Init) !void {
    ...
    var client = try zconnector.LlmClient.openai(init.gpa, api_key, base_url, io);
    defer client.deinit();
...
fn runAnthropic(init: std.process.Init) !void {
    ...
    var client = try zconnector.LlmClient.anthropic(init.gpa, api_key, base_url, io);
    defer client.deinit();
...
pub fn main() !void {
    runOpenAi(init) catch |err| {
        ...
        var writer = stderr_file.writer(io, &buffer);
